...
parser.add_option('-o', '--side', metavar='F', type='string', action='store',
default	=	'Right',
dest	=	'side',
help	=	'Right or Left')
...
config = mapsa.config(Config=1,string=options.setting+options.side,njump=0)
config.upload(show = 0, UL = wrongstring)
config.write(UL = wrongstring,n=n,m=m,c=c,u=u)
...
for x in range(0,256):
	...
	pix,mem = mapsa.daq().read_data(buffer_num=1,UL=wrongstring,nmpa =6)
	ipix=0
	for p in pix:
	...
	for iy1 in range(0,len(yarr[0,:])):
		...
		for ibin in range(0,len(xvec)-1):
			...
			if (yval1-halfmax)<0.0 and ibin>maxbin[0][0]:
				...
				xdacval = (abs(yval-halfmax)*xval1 + abs(yval1-halfmax)*xval)/(abs(yval-halfmax) + abs(yval1-halfmax))
...
for i in range(0,6):
	...
	print "writing data/Conf_calibrated"+options.side+"_MPA"+str(i+1)+"_config1.xml"
	xmlrootfile.write("data/Conf_calibrated"+options.side+"_MPA"+str(i+1)+"_config1.xml")
...
config1 = mapsa.config(Config=1,string='calibrated'+options.side,njump=njump)
confdict = {'OM':[3]*6,'RT':[0]*6,'SCW':[0]*6,'SH2':[0]*6,'SH1':[0]*6,'THDAC':[0]*6,'CALDAC':[options.charge]*6,'PML':[1]*6,'ARL':[1]*6,'CEL':[CE]*6,'CW':[0]*6,'PMR':[1]*6,'ARR':[1]*6,'CER':[CE]*6,'SP':[SP]*6,'SR':[1]*6,'TRIMDACL':[None]*6,'TRIMDACR':[None]*6}
config1.modifyfull(confdict) 

config1.upload(show = 0, UL = wrongstring)
config1.write(UL = wrongstring,n=n,m=m,c=c,u=u)
