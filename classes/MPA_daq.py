...
class MPA_daq:
	...
	def read_data(self,buffer_num,dcindex=-1,wait=True):
		self._Readout.getNode("Header").getNode("MPA"+str(self._nmpa)).write(0xFFFFFFFF)
		self._hw.dispatch()
		if dcindex==-1:
			dcindex=self._nmpa
		eshift = 0
	...
	def format(self,counter_data,memory_data,Fast=False):
		...
		pix = [None]*50
		mem = [None]*96
		#print "Header:      ",binary(counter_data[0])
		#print "HeaderTrue:  ", binary(0xFFFFFFFF)
		#print "Header2:     ",binary(counter_data[1])
		for x in range(0,25):
