...
class MPA_config:
	...
	def upload(self,show = 0,Config=1,dcindex=-1,UL = "Right"):
		if dcindex==-1:
			dcindex=self._nmpa
		cur = [None]*25
		...
		if (show):
			...
			bincur = []
			for cc in cur:
				bincur.append(binary(cc))
			print bincur
		self._spi_wait()

		self._Configuration1 =  self._hw.getNode("Configuration_" + UL)
		self._Memory_DataConf1   = self._Configuration1.getNode("Memory_DataConf")	

		self._Memory_DataConf1.getNode("MPA"+str(dcindex)).getNode("config_"+str(Config)).writeBlock(cur)
		#self._hw.getNode("Configuration_Left").getNode("num_MPA").write(0x1)
		#self._hw.dispatch()
		#self._spi_wait()
		return cur
	...
	def writeone(self,dcindex=1):
		...
		self._hw.dispatch()
		self._spi_wait()
		self._hw.getNode("Configuration_Left").getNode("mode").write(6-dcindex)
		self._hw.dispatch()


		self._Control.getNode("confs").write(0x1)
		self._hw.dispatch()
		self._Control.getNode("conf_upload").write(0x1)
		self._hw.dispatch()
...
###	
	...
	def write(self):
		...
		self._hw.getNode("Configuration_Right").getNode("mode").write(0x5)
		self._hw.dispatch()
		self._Control.getNode("confs").write(0x1)
		self._hw.dispatch()
		self._Control.getNode("conf_upload").write(0x1)
