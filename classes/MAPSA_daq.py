...
class MAPSA_daq:
	...
	def __init__(self, hw):
		...
		self._Configuration 	=  	self._hw.getNode("Configuration_Right")
		self._Readout 		=  	self._hw.getNode("Readout_Right")
...
#		self._hw.dispatch()
	...
	def header_init(self):
		for i in range(1,7):
			self._Readout.getNode("Header").getNode("MPA"+str(i)).write(0xF0F0F0F0)
			self._hw.dispatch()

	def read_data(self,buffer_num=1,UL="Right",nmpa =6,wait=True,Fast=False):
		counts = []  
		mems = []  



		self._Readout1 		=  	self._hw.getNode("Readout_"+UL)
		self._memory1  = self._Readout1.getNode("Memory")
		self._counter1  = self._Readout1.getNode("Counter")



		dcindex=-1

		for i in range(1,nmpa+1):
			#start = time.time()
			counter_data  = self._counter1.getNode("MPA"+str(i)).getNode("buffer_"+str(buffer_num)).readBlock(25)
			memory_data = self._memory1.getNode("MPA"+str(i)).getNode("buffer_"+str(buffer_num)).readBlock(216)
			...
			#print "read"
			#for imem in memory_data:
			#	print str(binary(imem))
